...
class sensenet(nn.Module):
    def __init__(self):
        ...
        self.glob_fc_1=nn.Linear(14016,30)
    ...
    def forward(self, x):
        ...
        g_out_3=F.max_pool2d(g_out_3, kernel_size=64)
        ...
        g_out_5=F.max_pool2d(g_out_5, kernel_size=64)
        ...
        g_out_7 = F.max_pool2d(g_out_7, kernel_size=64)
...
def data_loader(batch_size,class_lst,select_index):
    ...
    for n in select_index:
        for style in class_lst:
            ...
            new_mat=mat.copy()
            new_mat[:,:,0]=new_mat[:,:,0]-np.mean(new_mat[:,:,0])
            new_mat[:,:,1]=new_mat[:,:,1]-np.mean(new_mat[:,:,1])
            new_mat[:,:,2]=new_mat[:,:,2]-np.mean(new_mat[:,:,2])
            mat=new_mat.astype(np.uint8)
...
def train(epoch):
    ...
    for batch_idx, (input_data, input_target) in enumerate(train_loader):
        ...
        if batch_idx % log_interval == 0:
            with open("senselog.txt",'a') as file:
...
def test():
    ...
    with open('senseout.txt','a') as file:
        file.write('\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
        test_loss, correct, sub_number/4*30,
        100. * correct / (sub_number/4*30)))

test_loader=data_loader(batch_size,class_lst,np.arange(int(sub_number*0.95)+1,sub_number))
test()
